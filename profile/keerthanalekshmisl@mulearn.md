# Keerthana Lekshmi S L

**Squad Domain:** Coder | **FIFA Nation:** Brazil

## About Me

I am Keerthana Lekshmi S L, a B.Tech Information Technology student from Government Engineering College Idukki. I am passionate about web development, UI design, full-stack development, and building real-world projects that solve practical problems. I enjoy working with HTML, CSS, JavaScript, React, Node.js, MongoDB, GitHub, and learning cloud and DevOps concepts.

I joined μFIFA to learn by contributing, improve my public proof of work, collaborate with other learners, and become more confident in open-source and community-based challenges. I am still growing as a developer, but I want to use this tournament to build consistency, improve my skills, and make my name visible on the leaderboard.

---

## ⚽ FIFA World Cup Corner

### My Nation & Why

I am representing **Brazil** in μFIFA. I chose Brazil because of their beautiful style of football, strong World Cup history, attacking mindset, and legendary players. Brazil is a nation known for creativity, skill, confidence, and football passion. As a Neymar fan, Brazil feels like the perfect nation for me to represent.

### Supporting Team in the Real World Cup 2026

For the real FIFA World Cup 2026, I am supporting **Brazil**. My μFIFA nation and my real-world football support are the same. I want Brazil to come back strongly and show their classic attacking football on the world stage.

### All-Time Favourite Player

My all-time favourite footballer is **Neymar Jr.** I admire him because of his creativity, dribbling, flair, confidence, and ability to entertain fans. His playing style is unique, and he brings emotion and beauty to the game. As a fan, I connect with the way he plays with freedom and passion.

### Best Player Right Now

I think **Kylian Mbappé** is one of the best players in the world right now. His speed, finishing, confidence, and performance in big matches make him one of the most dangerous players in football.

### Past World Cup Memories

My most memorable FIFA World Cup moment was watching Brazil matches and seeing the passion of Brazilian football fans. Brazil’s style, energy, and emotional connection with football always make their World Cup matches special.

### 2026 Predictions

I think **Brazil** has a strong chance to win the FIFA World Cup 2026. My dark horse team to watch is **Morocco**, because they showed great fighting spirit and teamwork in the previous World Cup. The player who may define this tournament could be **Neymar Jr.** if he is fit and available, because his creativity and experience can make a big difference for Brazil. The match I am most looking forward to is **Brazil vs Argentina**, because it is one of the biggest rivalries in football.

### μFIFA World Cup 2026 - Tournament Goals

By the end of μFIFA, I want to improve my GitHub contribution habit, build a stronger public portfolio, learn more about open-source collaboration, and complete meaningful tasks. I want to contribute consistently, learn from other participants, and become more confident in presenting my work publicly.

---

## 🏅 Portfolio Highlights

### History of Open Source and Collaborative Contributions

I am currently starting my open-source journey through GitHub tasks, μLearn activities, and project-based learning. I have created and worked on repositories for web development projects, React applications, e-commerce websites, chat applications, portfolio pages, and DevOps learning tasks. During μFIFA, I want to improve my contribution history and learn how to make better pull requests.

### History of Community Engagement

I am part of learning communities where I participate in technical tasks, coding challenges, and project submissions. I have been engaging with μLearn activities and different web development and DevOps tasks. I am using these opportunities to improve my consistency, communication, and technical confidence.

### Domain Profiles

* **GitHub:** https://github.com/keerthu411
* **LinkedIn:** https://www.linkedin.com/in/keerthana-lekshmi-s-l-4374b63a1/
* **Portfolio:** https://keerthu411.github.io/keerthana-portfolio/

### Tools, Workflows & Automations

I use tools like VS Code, GitHub, Git, Chrome DevTools, React, Vite, Node.js, MongoDB, Firebase, Figma, Canva, ChatGPT, Google Colab, and basic DevOps tools. I am currently learning more about cloud, Docker, Linux commands, networking, and GitHub workflows.

### Public Portfolio & Recognition

I have worked on projects such as a personal portfolio website, BrownBelle e-commerce website, Smart Notes Pro, React Firebase Chat App, CineVerse movie listing app, YouTube clone, and DevOps basic project tasks. During μFIFA, I want to improve these projects, deploy more work publicly, and build stronger proof of work.

### Education and Proof of Work

I am pursuing **B.Tech in Information Technology** at **Government Engineering College Idukki**. I have completed learning activities and internships related to web development, data science, cybersecurity, and problem-solving. I am also building practical projects to improve my development and deployment skills.

* **GitHub:** https://github.com/keerthu411
* **LinkedIn:** https://www.linkedin.com/in/keerthana-lekshmi-s-l-4374b63a1/
* **Portfolio:** https://keerthu411.github.io/keerthana-portfolio/

### History of Leadership

I am still developing my leadership experience, but I have taken initiative in learning projects, completing technical tasks, helping with project work, and improving my skills through self-learning. I believe leadership starts with responsibility, consistency, and willingness to learn and contribute.

### Networking

I want to connect with coders, designers, makers, and strategists during μFIFA. I am interested in collaborating on web development projects, open-source tasks, portfolio-building activities, and beginner-friendly technical challenges. I want to learn from others and also share what I know.

### Career Plan

**Immediate Plan:**
In the next 6–12 months, I want to improve my skills in full-stack development, React, backend development, GitHub, cloud, and DevOps. I also want to complete more projects, deploy them, and build a strong portfolio for internships and placements.

**Long-Term Plan:**
My long-term goal is to become a skilled software developer with strong practical knowledge in full-stack development, cloud, and DevOps. μFIFA will help me build consistency, confidence, collaboration skills, and public proof of work.

---

## Profile Card

**Name:** Keerthana Lekshmi S L
**Squad Domain:** Coder
**FIFA Nation:** Brazil
**Institution:** Government Engineering College Idukki
**Field:** B.Tech Information Technology
**Focus Areas:** Web Development, Full Stack Development, UI Design, GitHub, Cloud, DevOps
**GitHub:** https://github.com/keerthu411
**LinkedIn:** https://www.linkedin.com/in/keerthana-lekshmi-s-l-4374b63a1/
**Portfolio:** https://keerthu411.github.io/keerthana-portfolio/
